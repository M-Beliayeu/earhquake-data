...
CREATE  OR REPLACE VIEW ViewForViz AS 
WITH 
MagTypeHelper AS
(
...
(
    ...
    FROM Agencies a LEFT JOIN Events e ON a.id = e.network
...
(
	SELECT im.global_id AS id, 
           ...
           aas.abb AS network, 
			ROW_NUMBER() OVER(PARTITION BY im.global_id ORDER BY aas.SCORE DESC) as rankcol
    FROM Events e
